...
coef0 = modela2$coefficients[1]
estLogNumHouse.f <- function(age){
  return(coefa2*(age)^2 + coefa*(age) + coef0)
}
...
kable(tab2chp4, booktabs=T,
      caption="Sample data for Exercise 15.") %>%
  kable_styling(full_width = F)
```
...
kable(tab3chp4, booktabs=T,
      caption="Data from Scotto et al. (1974) on the number of cases of non-melanoma skin cancer for women by age group in two metropolitan areas (Minneapolis-St. Paul and Dallas-Ft. Worth); the year is unknown.") %>%
  kable_styling(full_width = F) %>%
  add_footnote(c("The columns contain: number of cases, population size, age group, and city (1=Minneapolis-St. Paul, 2=Dallas-Ft. Worth)."), notation = "none")
```
